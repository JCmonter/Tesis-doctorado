...
Para consultar información adicional de la que aquí mencionaremos, recomendamos consultar \cite{Johnstone82} o \cite{PicadoPultr}. 
Algunas de las cuestiones espaciales que aquí serán mencionadas pueden encontrarse también en \cite{sexton2003point}, \cite{sexton2006point} y \cite{sexton2006ordinal}.\\

Algunos de los resultados que se muestran en este capítulo son muy conocidos, por tal motivo, los presentamos sin prueba. 
...
Como el título lo indica, esta sección está dedicada a presentar los conceptos básicos de la teoría de marcos. Como es usual, $\wedge$ y $\bigvee$ denotan las operaciones ínifimo finito y supremo arbitrario, respectivamente. 
De manera alternativa, podemos consultar también \cite{Zaldivar2023}.

\begin{dfn}\label{frm}
Un \emph{marco} es una retícula completa $(A, \leq, \wedge, \bigvee, 0, 1)$ que cumple la siguiente ley distributiva, la cual se conoce como  \emph{ley distributiva para marcos} \emph{(\textbf{LDM})}, y dice lo siguiente:
\begin{equation}\label{LDM}
x\wedge\bigvee Y=\bigvee\{x\wedge y\mid y\in Y\}
\end{equation}
para cualesquiera $x\in A$ y $Y\subseteq A$.
\end{dfn}

El ejemplo canónico de un marco proviene de la teoría de espacios topológicos. Para todo espacio topológico $S$ se tienen dos familias de subconjuntos: los subconjuntos abiertos, que denotamos por $\mathcal{O}S$ y sus subconjuntos cerrados, denotados por $\mathcal{C}S$. De esta forma para cualquier $(S,\mathcal{O}S)$, $\mathcal{O}S$ tiene la estructura 
\[
(\mathcal{O}S, \subseteq, \cap,\bigcup, S,\emptyset),
\]
y satiface \ref{LDM}, es decir, $\mathcal{O}S$ es un marco y se le conoce como el marco de abiertos de $S$.\\

Si consideramos $U\subseteq S$, denotamos por
...
Calcular ínfimos arbitrarios dentro de este marco se puede realizar de la siguiente forma 
\[
...
Utilizando las Definiciones \ref{frm} y \ref{morf} se demostra que la composición de morfismos de marcos es también un morfismo de marcos. Además, el morfismo identidad ($\id$) actúa como elemento neutro en la composición. Como consecuencia tenemos una categoría, la cual denotaremos por $\Frm$ y es conocida como la \emph{categoría de marcos}.
...
La categoría opuesta a $\Frm$ recibe el nombre de \emph{categoría de locales} y se denota $\Loc=\Frm^{\op}$. Por la Proposición \ref{Adjuntoder} tenemos que para cualesquiera $A, B\in \Frm$, si
\[
...
\]
tal que $f$ preserva ínfimos arbitrarios y su adjuntos izquierdo ($f^*$), es un morfismo de marcos.\\

La herramienta fundamental que relaciona a las categorías $\Frm$ y $\Top$ es la adjunción que existe entre ellas.
...
Las asignaciones entre objetos y flechas del funtor $\mathcal{O}$ se realiza de la siguiente manera: 
