...
\begin{proof}
    ...
    \begin{split}
    \text{Fuertemente apilado }&\iff u_d=u_a\\ 
    &\iff a=d\\ 
    &\iff v_F(0)=w_F(0)\\ 
    &\iff \text{ Apilado}.
    \end{split}
...
El resultado anterior nos dice que, en el caso espacial, KC y Eficiente son en esencia la misma propiedad. Ahora, debemos
...
\begin{proof}
    A manera de contradicción, supongamos que KC implica $\mathbf{(sfit)}$. Dado que KC implica $T_1$, entonces
    \[
    \text{KC}\Rightarrow T_1+ \mathbf{(sfit)}\Rightarrow \text{ Espacial},
    \]
    es decir, todos los marcos KC son espaciales.
    
    Lo anterior es una contradicción, pues el marco $A=B\times 2$, donde $B$ es un álgebra booleana completa sin átomos, es un marco 
    KC. En efecto, en el reporte anterior verificamos que
    \[
    A^\wedge=\{A, F_p\},
    \]
    para $p\in \pt A$. 

    Luego, dado que $A$ es $T_1$ y fuertemente apilado, se cumple que
    \[
    v_A=u_1=w_A\quad\text{y}\quad v_{F_p}=u_p=w_{F_p}.
    \]

    Por lo tanto, $A$ es KC, pero $A$ no es espacial. De esta manera, KC no implica $\mathbf{(sfit)}$.
